...
%A fundamentação teórica foi baseada no artigo {\it Earthquake Likelihood Model Testing}\cite{schorlemmer} desenvolvido pelo grupo {\it The Regional Earthquake Likelihood Models} (RELM). O artigo define três testes para modelos de previsão de sismos. Os testes N-test e L-test, são testes de consistência do modelo com os dados observados e o último é composto pelo R-test (também descrito a seguir), é uma comparação de todos os pares de modelos em um teste de consistência relativa, ambos baseados no método {\it likelihood}.\\
...
%A fundamentação teórica utilizada no presente trabalho foi baseada no artigo {\it Earthquake Likelihood Model Testing} \cite{schorlemmer2007earthquake} desenvolvido pelo grupo {\it The Regional Earthquake Likelihood Models} (RELM). O artigo define três testes estatísticos baseados no método {\it likelihood} que auxiliam na avalição de modelos de previsão de sismos. Os testes N-test e L-test, são testes de consistência do modelo com os dados observados e o último é composto pelo R-test (também descrito a seguir), é uma comparação de todos os pares de modelos em um teste de consistência relativa, ambos baseados no método {\it likelihood}.\\
